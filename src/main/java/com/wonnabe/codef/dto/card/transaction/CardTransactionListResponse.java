...
public class CardTransactionListResponse {
    private CardTransactionMeta result;
    private String endpoint;
    private List<CardTransactionItem> data;
    private String userId;
    private String connectedId;

    public List<CardTransaction> toCardTransactions(String userId,
                                                    String institutionCode,
                                                    AssetCardMapper assetCardMapper) {
        List<CardTransaction> list = new ArrayList<>();

        for (CardTransactionItem tx : data) {
    ...
    private CardTransaction.MerchantCategory mapMerchantCategory(String storeType) {
        if (storeType == null) return CardTransaction.MerchantCategory.other;

        String s = storeType.trim();
        if (s.isEmpty()) return CardTransaction.MerchantCategory.other;

        // 1) 정확 일치 우선
        if (FOOD_TYPES.contains(s)) return CardTransaction.MerchantCategory.food;
        if (TRANSPORT_TYPES.contains(s)) return CardTransaction.MerchantCategory.transport;
        if (SHOPPING_TYPES.contains(s)) return CardTransaction.MerchantCategory.shopping;
        if (CULTURE_TYPES.contains(s)) return CardTransaction.MerchantCategory.culture;

        // 2) 휴리스틱(부분문자열)
        // 음식점/전문점(안경 전문점은 쇼핑으로 분류)
        if (s.contains("음식점") || (s.endsWith("전문점") && !s.contains("안경"))) {
            return CardTransaction.MerchantCategory.food;
        }
        // 교통
        if (s.contains("택시") || s.contains("버스") || s.contains("지하철")
                || s.contains("항공") || s.contains("렌트") || s.contains("주유")) {
            return CardTransaction.MerchantCategory.transport;
        }
        ...
        if (s.contains("PG") || s.contains("상품권") || s.contains("오픈마켓")
            ...
            return CardTransaction.MerchantCategory.shopping;
        }
        // 문화(도서/공연/전시/영화/티켓 등)
        if (s.contains("영화") || s.contains("극장") || s.contains("공연") || s.contains("전시")
            ...
            return CardTransaction.MerchantCategory.culture;
        }

        return CardTransaction.MerchantCategory.other;
    }
}
