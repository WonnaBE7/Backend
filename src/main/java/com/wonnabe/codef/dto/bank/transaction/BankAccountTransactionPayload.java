...
public class BankAccountTransactionPayload {
    ...
    private List<BankAccountTransactionItem> historyList;

    public List<UserTransaction> toUserTransactions(String userId, String institutionCode, AccountMapper accountMapper) {
        List<UserTransaction> result = new ArrayList<>();
        if (historyList == null) return result;

        for (BankAccountTransactionItem detail : historyList) {
