...
@Log4j2
class InsuranceApplyControllerTest {
    ...
    public void setup() {
        ...
        userVO.setUserId("b2c3d4e5-f678-9012-abcd-ef12gh34ij56"); // 실제 DB에 존재하는 테스트용 사용자 ID
    ...
    @DisplayName("사용자 보험 신청 성공 테스트")
    void applyInsurance_success() throws Exception {
        // given: 테스트를 위한 요청 데이터 준비
        InsuranceApplyRequestDTO dto = InsuranceApplyRequestDTO.builder()
                .insuranceId("3002") // 실제 DB에 존재하는 테스트용 보험 상품 ID
    ...
    @Test
    @Transactional
    @DisplayName("잘못된 보험 ID로 신청 시 실패 테스트")
    void applyInsurance_fail_with_invalid_id() throws Exception {
        // given: 존재하지 않는 보험 ID로 요청 데이터 준비
        InsuranceApplyRequestDTO dto = InsuranceApplyRequestDTO.builder()
        ...
        // 왜냐하면 Service에서 발생한 NoSuchElementException을 ApiExceptionAdvice가 HTTP 404로 처리하기 때문
        mockMvc.perform(post("/api/users/insurance/apply")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(jsonRequest))
